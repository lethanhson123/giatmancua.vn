...
@Component({
  selector: 'app-doanh-nghiep-cahet-han',
  templateUrl: './doanh-nghiep-cahet-han.component.html',
  styleUrls: []
})
export class DoanhNghiepCAHetHanComponent implements OnInit {

  detailURL: string = environment.DoanhNghiepDetailURL;
  dataSource: MatTableDataSource<any>;
  displayColumns: string[] = ['actions', 'DateEnd', 'TaxCode', 'FullName'];
  ...
  getToList() {
    ...
    this.membershipService.getDoanhNghiepCAHetHanByDateEndAndCityIDAndWardIDAndProductIDToList(this.year, this.month, this.cityID, this.wardID).then(res => {
  ...
  onDownloadExcelFile() {
    this.isShowLoading = true;
    this.downloadService.getDoanhNghiepCAHetHanByDateEndAndCityIDAndWardIDAndProductIDToExcelURL(this.year, this.month, this.cityID, this.wardID).then(
      res => {
  ...
  onPrint() {
    this.isShowLoading = true;
    this.downloadService.getDoanhNghiepCAHetHanByDateEndAndCityIDAndWardIDAndProductIDToHTMLURL(this.year, this.month, this.cityID, this.wardID).then(
      res => {
        window.open(res.toString(), "_blank");
        this.isShowLoading = false;
      }
