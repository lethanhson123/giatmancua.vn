...
@Component({
  selector: 'app-doanh-nghiep-dich-vu',
  templateUrl: './doanh-nghiep-dich-vu.component.html',
  styleUrls: []
})
export class DoanhNghiepDichVuComponent implements OnInit {
  ...
  getToList() {
    ...
    let productIDList = environment.InitializationString;
    for (let i = 0; i < 100; i++) {
      if (this.productIDList[i] != null) {
        productIDList = productIDList + this.productIDList[i] + ';';
      }
    }
    this.membershipService.getDoanhNghiepKhongSuDungDichVuByProductIDListAndCityIDAndWardIDToList(productIDList, this.cityID, this.wardID).then(res => {
  ...
  onDownloadExcelFile() {
    ...
    this.downloadService.getDoanhNghiepKhongSuDungDichVuByProductIDListAndCityIDAndWardIDToExcelURL(productIDList, this.cityID, this.wardID).then(
      res => {
  ...
  onPrint() {
    ...
    this.downloadService.getDoanhNghiepKhongSuDungDichVuByProductIDListAndCityIDAndWardIDToHTMLURL(productIDList, this.cityID, this.wardID).then(
      res => {
        window.open(res.toString(), "_blank");
        this.isShowLoading = false;
      }
