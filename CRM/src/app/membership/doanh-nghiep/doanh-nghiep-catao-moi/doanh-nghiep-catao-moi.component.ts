...
@Component({
  selector: 'app-doanh-nghiep-catao-moi',
  templateUrl: './doanh-nghiep-catao-moi.component.html',
  styleUrls: []
})
export class DoanhNghiepCATaoMoiComponent implements OnInit {
  ...
  getToList() {
    ...
    this.membershipService.getDoanhNghiepCATaoMoiByDateBeginAndCityIDAndWardIDAndProductIDToList(this.year, this.month, this.cityID, this.wardID).then(res => {
  ...
  onDownloadExcelFile() {
    this.isShowLoading = true;
    this.downloadService.getDoanhNghiepCATaoMoiByDateBeginAndCityIDAndWardIDAndProductIDToExcelURL(this.year, this.month, this.cityID, this.wardID).then(
      res => {
  ...
  onPrint() {
    this.isShowLoading = true;
    this.downloadService.getDoanhNghiepCATaoMoiByDateBeginAndCityIDAndWardIDAndProductIDToHTMLURL(this.year, this.month, this.cityID, this.wardID).then(
      res => {
        window.open(res.toString(), "_blank");
        this.isShowLoading = false;
      }
