...
@Component({
  selector: 'app-doanh-nghiep-cadang-hoat-dong',
  templateUrl: './doanh-nghiep-cadang-hoat-dong.component.html',
  styleUrls: []
})
export class DoanhNghiepCADangHoatDongComponent implements OnInit {
  ...
  getToList() {
    ...
    this.membershipService.getDoanhNghiepCADangHoatDongByDateEndAndCityIDAndWardIDAndProductIDToList(this.cityID, this.wardID).then(res => {
  ...
  onDownloadExcelFile() {
    this.isShowLoading = true;
    this.downloadService.getDoanhNghiepCADangHoatDongByCityIDAndWardIDAndProductIDToExcelURL(this.cityID, this.wardID).then(
      res => {
  ...
  onPrint() {
    this.isShowLoading = true;
    this.downloadService.getDoanhNghiepCADangHoatDongByCityIDAndWardIDAndProductIDToHTMLURL(this.cityID, this.wardID).then(
      res => {
        window.open(res.toString(), "_blank");
        this.isShowLoading = false;
      }
